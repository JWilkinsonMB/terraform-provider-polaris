...
- `skip_validation` (Boolean) Skip validation of the Exocompute configuration by Rubrik Security Cloud prior to creation. This is needed where Private DNS zones are used and they exist outside of the subscription in which Exocompute is being deployed as this configuration must be managed outside of Rubrik Security Cloud and cannot be validated. Changing this forces a new resource to be created. Defaults to `false`.
...
- `trigger_health_check` (Boolean) Trigger a health check for the Exocompute configuration. Changing this forces a new resource to be created. Defaults to `false`.
